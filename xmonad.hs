...
myStartupHook = do
  ...
  spawnOnce "feh --randomize --bg-fill /home/jason/Pictures/wallpapers/*"  -- feh set random wallpaper
...
myWorkspaces = [" term ", " www ", " file ", " doc ", " emacs ", " chat ", " music ", " vid ", " tor "]
  ...
  xmonad $ addDescrKeys' ((mod4Mask, xK_F1), showKeybindings) myKeys $ ewmh $ ewmhFullscreen $ docks $ def -- Just remove $ ewmhFullscreen if YouTube fullscreen doesn't work
    ...
    , logHook = dynamicLogWithPP $  filterOutWsPP [scratchpadWorkspaceTag] $ xmobarPP
        { ppOutput = \x -> hPutStrLn xmproc0 x   -- xmobar on monitor 1
                        >> hPutStrLn xmproc1 x   -- xmobar on monit
